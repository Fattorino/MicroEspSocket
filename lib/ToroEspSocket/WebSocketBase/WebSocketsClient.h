...
public:
...
#else
    typedef std::function<void(TES_Client *th, WStype_t type, uint8_t *payload, size_t length)> WebSocketClientEvent;
#endif
    ...
    void onEvent(TES_Client *tesClient, WebSocketClientEvent cbEvent);
...
#define SSL_FINGERPRINT_NULL NULL
    ...
    virtual void runCbEvent(WStype_t type, uint8_t *payload, size_t length)
    {
        if (_cbEvent)
        {
            _cbEvent(_tesClient, type, payload, length);
        }
    }
};

...
#else
    typedef std::function<void(TES_Server *tesServer, uint8_t num, WStype_t type, uint8_t *payload, size_t length)> WebSocketServerEvent;
    typedef std::function<bool(String headerName, String headerValue)> WebSocketServerHttpHeaderValFunc;
#endif

    void onEvent(TES_Server *tesServer, WebSocketServerEvent cbEvent);
...
protected:
    ...
    uint _websocketMaxClient;

    // WSclient_t _clients[WEBSOCKETS_SERVER_CLIENT_MAX];
    std::vector<WSclient_t> _clients;

    WebSocketServerEvent _cbEvent;
    TES_Server *_tesServer;
    ...
    virtual void runCbEvent(uint8_t num, WStype_t type, uint8_t *payload, size_t length)
    {
        if (_cbEvent)
        {
            _cbEvent(_tesServer, num, type, payload, length);
        }
    }
...
{
public:
    WebSocketsServer(uint16_t port, uint maxClientConnectec, const String &origin = "", const String &protocol = "arduino");

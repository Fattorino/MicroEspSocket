...
WebSocketsServer::WebSocketsServer(uint16_t port, uint maxClientConnectec, const String &origin, const String &protocol)
    : WebSocketsServerCore(origin, protocol)
{
    _port = port;
    _websocketMaxClient = maxClientConnectec;
 ...
 */
void WebSocketsServerCore::onEvent(TES_Server *tesServer, WebSocketServerEvent cbEvent)
{
    _cbEvent = cbEvent;
    _tesServer = tesServer;
}
...
bool WebSocketsServerCore::broadcastTXT(uint8_t *payload, size_t length, bool headerToPayload)
{
    ...
    for (uint8_t i = 0; i < _websocketMaxClient; i++)
    {
        client = &_clients[i];
        if (clientIsConnected(client))
        {
